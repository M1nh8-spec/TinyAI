...
starts the terminal chat. The only command-line switches are --cpu and --cuda.
"""
import argparse
import os
import subprocess
...
def ensure_local_environment():
    """Create lib/, install project dependencies, then re-run inside it."""
    executable = venv_python()
    running_in_local_venv = os.path.abspath(sys.prefix) == os.path.abspath(VENV_DIR)
    if not running_in_local_venv:
        if not os.path.isfile(executable):
            print(f"Creating local virtual environment: {VENV_DIR}")
            venv.EnvBuilder(with_pip=True, clear=False).create(VENV_DIR)
        print("Checking/installing dependencies in lib ...")
...
import torch  # noqa: E402  (intentionally imported after lib bootstrap)
...
def train_if_needed(checkpoint_path, device):
    ...
    result = subprocess.run(command, check=False, cwd=PROJECT_DIR)
...
def main():
    ...
    while True:
        ...
        answer = generate(model, tokenizer, context, device,
