...
class CausalSelfAttention(nn.Module):
    def __init__(self, d_model, heads, dropout, context):
        ...
        self.register_buffer(
            "mask",
            torch.tril(torch.ones(context, context, dtype=torch.bool)),
            persistent=False,
        )
    ...
    def forward(self, x):
        ...
        # weights: [batch, heads, query_tokens, key_tokens]
        # v:       [batch, heads, key_tokens, head_dim]
        attended = weights @ v
        attended = attended.transpose(1, 2).contiguous().view(batch, tokens, channels)
        return self.out(self.drop(attended))
...
class MiniTransformer(nn.Module):
    def __init__(self, config):
        ...
        # Weight tying is intentional and still randomly initialized.
